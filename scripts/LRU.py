...
def bootstrapped_frame_potential():

    ansatze_name = "Local_Random"
    ...
    allfiles = os.listdir('../results/' + ansatze_name)
    files = []
    for file in allfiles:
        if file.endswith('.pt'):
            files.append(file)
    n_pattern = re.compile(r'n_\d*')
    l_pattern = re.compile(r'l_(\d+(?:\.\d+)?)')
    for n in range(max_ns+1):
        for l in range(max_l):
            tensor = torch.empty((0)).to('cpu')
            
            for file in files:
                n_match = n_pattern.findall(file)
                if len(n_match) != 0:
                    if n == int(n_match[0][2:]):
                        l_match = l_pattern.findall(file)
                        if l/4 == float(l_match[0]):
                            tensor = torch.cat((tensor, torch.load(results_dir + file)))
