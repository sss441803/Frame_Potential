...
ansatze_name = 'Hardware_Efficient_cZ'
...
class HardwareEfficientComposer(ParallelComposer):
    ...
    def entangling_layer(self):
        for i in range(self.n_qubits//2):
            control_qubit = 2*i
            target_qubit = 2*i+1
            self.apply_gate(self.operators.cZ, control_qubit, target_qubit)
...
class HE_trace_mod(HybridModule):
    ...
    def __init__(self, n_qubits, n_layers, optimizer=DefaultOptimizer):
        circuit_name = 'HE_cZ_trace_n_{}_l_{}'.format(n_qubits, n_layers)
