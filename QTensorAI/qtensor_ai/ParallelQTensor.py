...
class ParallelTorchBuilder(QtreeBuilder):
    ...
    def inverse(self):
        ...
        def dagger_gate(gate):
            ...
            if not gate.is_inverse:
                new.name = gate.name + '+'
                new.is_inverse = True
            else:
                new.name = gate.name[:-1]
                new.is_inverse = False
...
class ParallelTorchQkernelComposer(CircuitComposer):
    ...
    def variational_layer(self, layer_params):
        for i in range(self.n_qubits):
            qubit = self.qubits[i]
...
class InnerProductCircuitComposer(ParallelTorchQkernelComposer):
    ...
    def entangling_layer(self, is_inverse):
        if is_inverse:
            for i in range(self.n_qubits//2):
                control_qubit = self.qubits[2*i]
                target_qubit = self.qubits[2*i+1]
    ...
    def updated_expectation_circuit(self, params1, params2):
        ...
        self.circuit(params1)
        first_part = self.builder.circuit
        self.builder.reset()
        self.circuit(params2)
        self.builder.inverse()
        second_part = self.builder.circuit
        self.builder.reset()
        return first_part + second_part
...
'''Learnable embedding according to Lloyd et. al. arXiv:2001.03622'''
...
'''First layer changed to RY. Learnable embedding according to Lloyd et. al. arXiv:2001.03622'''
...
'''Random parameterized quantum circuits (RPQCs) according to https://arxiv.org/pdf/1803.11173.pdf'''
class RPQCComposer(ParallelTorchQkernelComposer):
    ...
    def variational_layer(self, layer_gate_types, layer_params):
        for i in range(self.n_qubits):
            qubit = self.qubits[i]
            self.apply_gate(self.operators.PauliPhase, qubit, alpha=layer_params[:, i], gates=layer_gate_types[:, i], device=self.device, is_placeholder = self.expectation_circuit_initialized)
    ...
    def updated_expectation_circuit(self, gate_types, params):
        self.builder.reset()
        self.device = params.device
        self.circuit(gate_types, params)
        first_part = self.builder.circuit
        self.builder.reset()
        return first_part

    def expectation_circuit(self, gate_types, params):
        new_circuit = self.updated_expectation_circuit(gate_types, params)

        if len(self.static_circuit) != 0:
            for self_op, new_op in zip(self.static_circuit, new_circuit):
                if isinstance(self_op, ParallelParametricGate):
                    self_op._parameters['alpha'] = new_op._parameters['alpha']
                    self_op._parameters['gates'] = new_op._parameters['gates']
                else:
                    self_op._parameters['n_batch'] = new_op._parameters['n_batch']
        else:
            '''Initializing circuit'''
...
class TraceEvaluationCircuitComposer(ParallelTorchQkernelComposer):
    ...
    def added_circuit(self):
        for target_qubit in range(self.n_target_qubits):
            ...
            self.apply_gate(self.operators.H, control_qubit, n_batch=self.n_batch, device=self.device, is_placeholder = self.expectation_circuit_initialized)
            self.apply_gate(self.operators.cX, control_qubit, target_qubit, n_batch=self.n_batch, device=self.device, is_placeholder = self.expectation_circuit_initialized)

    '''Building circuit whose first amplitude is the expectation value of the measured circuit wrt to the cost_operator'''
    def expectation_circuit(self, circuit):
        self.builder.reset()
        self.device = circuit[0]._parameters['device']
        self.n_batch = circuit[0]._parameters['n_batch']
        self.added_circuit()
        first_part = self.builder.circuit
        self.builder.inverse()
        second_part = self.builder.circuit
    ...
    def inverse(circuit):
        ...
        return list(reversed([dagger_gate(g) for g in circuit]))
